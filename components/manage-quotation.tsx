...
import { Loader2, FileText, Search, X, PlusCircle, List, CheckCircle, XCircle, RefreshCw, Clock, AlertTriangle, DollarSign, ChevronDown, ChevronRight } from "lucide-react"
...
type TabType = "create" | "list" | "total-bill"
...
function generateQuotationId(engineer: UserData | undefined, count: number): string {
  const year = String(new Date().getFullYear())
  const seq = String(count + 1).padStart(2, "0")
  if (engineer && branchRoles.includes(engineer.role)) {
    return `${seq}-${engineer.branch?.name || "Unknown"}-${year}`
  }
  return `${seq}-HO-${year}`
}
...
function CreateQuotationTab({ onSuccess }: { onSuccess: () => void }) {
  ...
  const fetchNextId = useCallback(async () => {
    try {
      ...
      const selectedEngineer = users.find((u) => u.name === formData.engineerName)
      setFormData((p) => ({ ...p, quotationId: generateQuotationId(selectedEngineer, count) }))
    } catch {
      setFormData((p) => ({ ...p, quotationId: generateQuotationId(undefined, 0) }))
    }
  }, [users, formData.engineerName])
  ...
  useEffect(() => {
    ...
    fetch("/api/users", { credentials: "include" })
      .then((r) => r.json())
      .then((data) => {
        if (data.error) { console.error("Users API error:", data.error); return }
        if (data.users) {
          const filtered = data.users.filter((u: UserData) => [...hoRoles, ...branchRoles].includes(u.role))
          console.log(`Fetched ${data.users.length} users, ${filtered.length} match engineer roles`)
          setUsers(filtered)
        }
      })
    ...
    fetch("/api/companiesforservice", { credentials: "include" })
      ...
      .catch(() => {})
    fetch("/api/dropdowns?kind=quotation_type", { credentials: "include" })
      .then((r) => r.json())
      .then((d) => { if (d.options) setQuotationTypes(d.options.map((o: { label: string }) => o.label)) })
      .catch(() => {})
    fetch("/api/dropdowns?kind=department", { credentials: "include" })
      .then((r) => r.json())
      .then((d) => { if (d.options) setDepartments(d.options.map((o: { label: string }) => o.label)) })
      .catch(() => {})
  }, [fetchNextId])
  ...
  const selectCompany = (company: CompanyData) => {
    setFormData((p) => ({
      ...
      contactPerson: company.locations?.[0]?.contactPerson || company.contactPerson || "",
      contactNumber: company.locations?.[0]?.contactNumber || company.contactNumber || "",
    }))
  ...
  const selectLocation = (loc: { location: string; contactPerson: string; contactNumber: string }) => {
    setFormData((p) => ({
      ...p,
      contactPerson: loc.contactPerson || p.contactPerson,
      contactNumber: loc.contactNumber || p.contactNumber,
    }))
    setLocationPickerOpen(false)
  }

  const clearCustomer = () => {
    setFormData((p) => ({ ...p, customerName: "", contactPerson: "", contactNumber: "" }))
    setCustomerSearch("")
  }
  ...
  const selectEngineer = (name: string, id: string) => {
    setFormData((p) => ({ ...p, engineerName: name, engineer: id }))
    setEngineerSearch(name)
    setEngineerDropdownOpen(false)
  }

  const clearEngineer = () => {
    setFormData((p) => ({ ...p, engineerName: "", engineer: "" }))
    setEngineerSearch("")
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setEngineerSearch(""); setCustomerSearch("")
        fetchNextId()
        onSuccess()
      } else {
  ...
  const canSubmit = formData.quotationId && formData.customerName && formData.engineerName && formData.amount && formData.quotationType && formData.quotationDate
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label htmlFor="quotationId">ID</Label>
              <Input id="quotationId" value={formData.quotationId} readOnly className="bg-slate-50 text-slate-600 cursor-not-allowed" />
            </div>
            <div>
              <Label htmlFor="quotationDate">Quotation Date</Label>
              <Input id="quotationDate" type="date" value={formData.quotationDate} onChange={(e) => handleChange("quotationDate", e.target.value)} required />
            </div>
            <div ref={customerRef}>
              ...
              <div className="relative">
                <div className="relative">
                  ...
                  <Input id="customerName" value={customerSearch} onChange={(e) => { setCustomerSearch(e.target.value); setCustomerDropdownOpen(true); if (formData.customerName) setFormData((p) => ({ ...p, customerName: "", contactPerson: "", contactNumber: "" })) }} onFocus={() => setCustomerDropdownOpen(true)} placeholder={selectedCompany ? selectedCompany.name : "Search or enter customer name..."} className="pl-9 pr-9" required />
              ...
              {selectedCompany && companyLocations.length > 1 && (
                <div ref={locationRef} className="mt-2">
                  ...
                  <div className="relative">
                    <Input
                      value={formData.contactPerson || formData.contactNumber ? `${formData.contactPerson}${formData.contactPerson && formData.contactNumber ? " — " : ""}${formData.contactNumber}` : "Select a location..."}
            ...
            <div>
              ...
              <div className="relative">
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                  <Input id="engineerName" value={engineerSearch} onChange={(e) => { setEngineerSearch(e.target.value); setEngineerDropdownOpen(true); if (formData.engineerName) setFormData((p) => ({ ...p, engineerName: "" })) }} onFocus={() => setEngineerDropdownOpen(true)} placeholder={selectedEngineer ? selectedEngineer.name : "Search for an engineer..."} className="pl-9 pr-9" required />
                  {selectedEngineer && <button type="button" onClick={clearEngineer} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"><X className="h-4 w-4" /></button>}
                </div>
                {engineerDropdownOpen && (
                  <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-y-auto">
                    {filteredEngineers.length === 0 ? <div className="px-3 py-2 text-sm text-gray-500">No engineers found</div> : filteredEngineers.map((engineer) => (
                      <button key={engineer._id} type="button" onClick={() => selectEngineer(engineer.name, engineer._id)} className={cn("w-full text-left px-3 py-2 text-sm hover:bg-blue-50 transition-colors", formData.engineerName === engineer.name ? "bg-blue-50 text-blue-700" : "text-gray-900")}>
              ...
              {selectedEngineer && <p className="mt-1 text-xs text-green-600">Selected: {selectedEngineer.name}</p>}
            </div>
            <div>
              <Label htmlFor="amount">Amount</Label>
              <Input id="amount" type="number" step="0.01" min="0" value={formData.amount} onChange={(e) => handleChange("amount", e.target.value)} placeholder="Enter amount" required />
            </div>
            <div>
            ...
            <div ref={departmentRef}>
              ...
              <div className="relative">
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                  <Input value={departmentSearch} onChange={(e) => { setDepartmentSearch(e.target.value); setDepartmentDropdownOpen(true); if (formData.department) setFormData((p) => ({ ...p, department: "" })) }} onFocus={() => setDepartmentDropdownOpen(true)} placeholder={selectedDepartment || "Search department..."} className="pl-9 pr-9" />
                  {selectedDepartment && <button type="button" onClick={clearDepartment} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"><X className="h-4 w-4" /></button>}
                </div>
                {departmentDropdownOpen && (
                  <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-y-auto">
                    {filteredDepartments.length === 0 ? <div className="px-3 py-2 text-sm text-gray-500">No departments found</div> : filteredDepartments.map((dept) => (
                      <button key={dept} type="button" onClick={() => selectDepartment(dept)} className={cn("w-full text-left px-3 py-2 text-sm hover:bg-blue-50 transition-colors", formData.department === dept ? "bg-blue-50 text-blue-700" : "text-gray-900")}>
            ...
            <div>
              ...
              <Input id="contactPerson" value={formData.contactPerson} onChange={(e) => handleChange("contactPerson", e.target.value)} placeholder="Enter contact person" required />
            </div>
            <div>
              <Label htmlFor="contactNumber">Contact Number</Label>
              <Input id="contactNumber" value={formData.contactNumber} onChange={(e) => handleChange("contactNumber", e.target.value)} placeholder="Enter contact number" required />
            </div>
          </div>
          <Button type="submit" className="w-full" disabled={!canSubmit || submitting}>
            {submitting ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Creating...</> : <><FileText className="w-4 h-4 mr-2" />Create Quotation</>}
          </Button>
        </form>
      </CardContent>
    </Card>
...
function QuotationsListTab({ refreshKey }: { refreshKey: number }) {
  ...
  useEffect(() => {
    if (!user) return
    setLoading(true)
    fetch(`/api/quotations?createdBy=${user._id}`, { credentials: "include" })
      .then((r) => r.json())
  ...
  const uniqueCustomers = [...new Set(quotations.map((q) => q.customerName))].filter(Boolean).sort()
  const uniqueDepartments = [...new Set(quotations.map((q) => q.department).filter(Boolean))].sort() as string[]
  const uniqueEngineers = [...new Set(quotations.map((q) => q.engineerName))].filter(Boolean).sort()
  const uniqueStatuses = [...new Set(quotations.map((q) => q.status))].filter(Boolean).sort()
  ...
  return (
    <Card>
      ...
      <CardContent>
        ...
        <p className="text-xs text-slate-400 mb-3">{filteredQuotations.length} of {quotations.length} quotations</p>
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-sm text-slate-400 mt-1">Create your first quotation from the Create tab</p>
          </div>
        ) : (
...
function TotalBillTab() {
  ...
  const filteredRawData = rawData.filter((q) => {
    if (selectedEngineer && q.engineerName !== selectedEngineer) return false
  ...
  const filteredEngineers = uniqueEngineers.filter((name) => name.toLowerCase().includes(engineerSearch.toLowerCase()))
  ...
  const hasActiveFilters = selectedEngineer || filterDept || filterDateFrom || filterDateTo
  const clearAllFilters = () => {
    clearEngineer()
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>View bill summary by engineer</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="flex flex-wrap gap-3 mb-4 items-end">
          <div ref={dropdownRef} className="relative min-w-[180px]">
            ...
            <div className="relative">
              <Search className="absolute left-2 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-gray-400" />
              <Input value={engineerSearch} onChange={(e) => { setEngineerSearch(e.target.value); setDropdownOpen(true); if (selectedEngineer) setSelectedEngineer("") }} onFocus={() => setDropdownOpen(true)} placeholder={selectedEngineer || "All engineers"} className="pl-7 pr-7 h-8 text-xs" />
              {selectedEngineer && <button type="button" onClick={clearEngineer} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"><X className="h-3 w-3" /></button>}
            </div>
            {dropdownOpen && (
              <div className="absolute z-20 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-48 overflow-y-auto">
                <button type="button" onClick={() => { setSelectedEngineer(""); setEngineerSearch(""); setDropdownOpen(false) }} className="w-full text-left px-3 py-1.5 text-xs hover:bg-blue-50 text-gray-500">All engineers</button>
                {filteredEngineers.map((name) => (
                  <button key={name} type="button" onClick={() => selectEngineer(name)} className={cn("w-full text-left px-3 py-1.5 text-xs hover:bg-blue-50 transition-colors", selectedEngineer === name ? "bg-blue-50 text-blue-700 font-medium" : "text-gray-900")}>{name}</button>
                ))}
              </div>
            )}
          </div>
...
export default function ManageQuotationPage() {
  ...
  const tabs = [
    { id: "create" as TabType, label: "Create Quotation", icon: PlusCircle },
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      {activeTab === "create" && <CreateQuotationTab onSuccess={() => setRefreshKey((k) => k + 1)} />}
      {activeTab === "list" && <QuotationsListTab refreshKey={refreshKey} />}
      {activeTab === "total-bill" && <TotalBillTab />}
    </div>
  )
}
