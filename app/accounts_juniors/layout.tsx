...
export default function AccountsJuniorsLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (mounted && !useAuthStore.getState().isLoading && !user) {
      router.push("/login")
    } else if (mounted && user && user.role !== "accounts_juniors" && user.role !== "admin") {
  ...
  const navItems = [
    { href: "/accounts_juniors/lead-transfer", label: "Lead Transfer", icon: ArrowRightLeft },
    { href: "/accounts_juniors/profile", label: "Profile", icon: User },
  ]
