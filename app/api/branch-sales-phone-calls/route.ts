...
export async function POST(request: Request) {
  try {
    ...
    const call = new BranchSalesPhoneCall({
    ...
    console.error("Save branch sales phone call error:", error)
...
export async function GET(request: Request) {
  try {
    ...
    const calls = await BranchSalesPhoneCall.find(query)
      .populate("userId", "name email")
      .sort({ createdAt: -1 })

    return NextResponse.json({ calls })
  } catch (error: any) {
    console.error("Get branch sales phone calls error:", error)
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
}
