...
export default function BranchLayout({ children, role, title, subtitle }: BranchLayoutProps) {
  ...
  useEffect(() => {
    if (mounted && !useAuthStore.getState().isLoading && !user) {
      router.push("/login")
    } else if (mounted && user && user.role !== role && user.role !== "admin") {
  ...
  const navItems = [
    { href: `/${role}/tracking`, label: "Sales / Service / Consumable Tracking", icon: LayoutDashboard },
    { href: `/${role}/lead-transfer`, label: "Lead Transfer", icon: ArrowRightLeft },
    { href: `/${role}/task-assign`, label: "Call/Case Assign", icon: ClipboardList },
    { href: `/${role}/manage-quotation`, label: "Manage Quotation", icon: FileText },
    { href: `/${role}/company-list`, label: "Company List", icon: MapPin },
    { href: `/${role}/travel-time`, label: "Travel Time", icon: Clock },
    { href: `/${role}/profile`, label: "Profile", icon: User },
  ]
