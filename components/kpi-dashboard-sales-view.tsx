...
export default function SalesKpiDashboardView() {
  ...
  const [selectedMetricId, setSelectedMetricId] = useState<string>("cc")
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [targetsRes, activitiesRes] = await Promise.all([
        axios.get(`/api/kpi-targets?month=${month}&year=${year}`, {
  ...
  const buildMetrics = (acts: Activity[], t: KpiTarget, factor: number): KpiMetric[] => {
