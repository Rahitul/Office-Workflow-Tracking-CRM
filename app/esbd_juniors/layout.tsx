...
import { LayoutDashboard, ArrowRightLeft, LogOut, Menu, X, User, GraduationCap, ClipboardList, FileText, Monitor } from "lucide-react"
import { cn } from "@/lib/utils"
import NotificationBell from "@/components/notification-bell"

export default function EsbdJuniorsLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (mounted && !useAuthStore.getState().isLoading && !user) {
      router.push("/login")
    } else if (mounted && user && user.role !== "esbd_juniors" && user.role !== "admin") {
  ...
  const navItems = [
    { href: "/esbd_juniors/trainings", label: "My Trainings", icon: GraduationCap },
    { href: "/esbd_juniors/lead-transfer", label: "Lead Transfer", icon: ArrowRightLeft },
    { href: "/esbd_juniors/task-assign", label: "Call/Case List", icon: ClipboardList },
    { href: "/esbd_juniors/machine-list", label: "Machine List / Service Card", icon: Monitor },
    { href: "/esbd_juniors/manage-quotation", label: "Manage Quotation", icon: FileText },
    { href: "/esbd_juniors/profile", label: "Profile", icon: User },
  ]
