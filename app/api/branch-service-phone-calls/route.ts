...
export async function POST(request: Request) {
  try {
    ...
    const call = new BranchServicePhoneCall({
    ...
    console.error("Save branch service phone call error:", error)
...
export async function GET(request: Request) {
  try {
    ...
    const calls = await BranchServicePhoneCall.find(query)
      .populate("userId", "name email")
      .sort({ createdAt: -1 })

    return NextResponse.json({ calls })
  } catch (error: any) {
    console.error("Get branch service phone calls error:", error)
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
}
