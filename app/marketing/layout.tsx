...
export default function MarketingLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (mounted && !useAuthStore.getState().isLoading && !user) {
      router.push("/login")
    } else if (mounted && user && user.role !== "marketing" && user.role !== "admin") {
  ...
  const navItems = [
    ...
    { href: "/marketing/trainings", label: "My Trainings", icon: GraduationCap },
    { href: "/marketing/lead-transfer", label: "Lead Transfer", icon: ArrowRightLeft },
    { href: "/marketing/profile", label: "Profile", icon: User },
  ]
