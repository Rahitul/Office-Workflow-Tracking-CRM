...
export default function EsbdLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (mounted && !useAuthStore.getState().isLoading && !user) {
      router.push("/login")
    } else if (mounted && user && user.role !== "esbd" && user.role !== "admin") {
  ...
  const navItems = [
    { href: "/esbd/dashboard", label: "Dashboard", icon: LayoutDashboard },
    { href: "/esbd/brands", label: "Brands", icon: Building2 },
    { href: "/esbd/products", label: "Products", icon: Package },
