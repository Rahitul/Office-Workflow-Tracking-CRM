...
export default function SalesDashboardPage() {
  ...
  const fetchEntries = useCallback(async () => {
    try {
      ...
      if (selectedTarget) params.append("targetId", selectedTarget)
      if (month) params.append("month", month)
      if (year) params.append("year", year)
      if (startDate) params.append("startDate", startDate)
      if (endDate) params.append("endDate", endDate)
      
      const response = await axios.get(`/api/daily-sales?${params.toString()}`, { withCredentials: true })
      setEntries(response.data.entries || [])
    } catch (error) {
      console.error("Failed to fetch entries:", error)
    }
  }, [targetType, selectedTarget, month, year, startDate, endDate])

  useEffect(() => {
    fetchSalesmen()
    fetchTargets()
    fetchEntries()
  }, [fetchSalesmen, fetchTargets, fetchEntries])
  ...
  const filteredTargets = useMemo(() => {
    ...
    return targets.filter(t => {
      ...
      if (targetType && t.targetType !== targetType) return false
      if (selectedTarget && t.targetId !== selectedTarget) return false
      return true
    })
  }, [targets, month, year, targetType, selectedTarget])
  ...
  return (
    <div className="space-y-8 pb-12">
      <div>
        ...
        <p className="text-slate-500">View sales progress and entries with filters</p>
      </div>
