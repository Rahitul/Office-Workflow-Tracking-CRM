...
export async function POST(request: Request) {
  try {
    ...
    const result = await BranchConsumableActivity.findOneAndUpdate(filter, update, {
      upsert: true,
      new: true,
    ...
    console.error("Branch consumable activity POST error:", error)
...
export async function GET(request: Request) {
  try {
    ...
    const activities = await BranchConsumableActivity.find(query)
      .populate("userId", "name email")
      .sort({ activityDate: -1 })

    return NextResponse.json({ activities })
  } catch (error: any) {
    console.error("Branch consumable activity GET error:", error)
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
}
