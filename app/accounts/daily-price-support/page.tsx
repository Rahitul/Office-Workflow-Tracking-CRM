...
export default function DailyPriceSupportPage() {
  const [loading, setLoading] = useState(false)
  const [targets, setTargets] = useState<PriceSupportTarget[]>([])
  ...
  const fetchEntries = async () => {
    try {
      const response = await axios.get("/api/daily-price-supports", { withCredentials: true })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedProject || !month || !year || !entryDate || !amountBDT || !gpMargin || !executiveName) {
    ...
    try {
      ...
      const relatedTarget = targets.find(t => 
        t._id === selectedProject
      )

      if (!relatedTarget) {
        setErrorMessage("Please select a valid price support target.")
        setLoading(false)
        return
      }

      await axios.post("/api/daily-price-supports", {
        priceSupportTargetId: relatedTarget._id,
        projectClientName: relatedTarget.projectClientName,
        month: monthNum,
        year: yearNum,
        entryDate,
  ...
  const projectOptions = useMemo(() => {
    return filteredTargets.map(t => ({
      value: t._id,
      label: `${t.projectClientName} - ${t.executiveName}`,
    }))
  }, [filteredTargets])
  ...
  const selectedTarget = useMemo(() => {
    return targets.find(t => t._id === selectedProject)
  ...
  const targetProgress = useMemo(() => {
    ...
    return filteredTargets.map(target => {
      const targetEntries = entries.filter(e => 
        e.projectClientName === target.projectClientName && 
        e.month === monthNum && 
        e.year === yearNum
      )
  ...
  useEffect(() => {
    if (selectedTarget && !gpMargin) {
      setGpMargin(selectedTarget.gpMargin.toString())
    }
    if (selectedTarget && !executiveName) {
      setExecutiveName(selectedTarget.executiveName)
    }
  }, [selectedTarget])
  ...
  return (
    <div className="space-y-8 pb-12">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Daily Price Support</h1>
        <p className="text-slate-500">Input daily price support amount</p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div className="flex items-end">
                <Button
                  type="submit"
                  disabled={loading || !selectedProject || !month || !year || !entryDate || !amountBDT || !gpMargin || !executiveName}
                    ...
                    <div className="p-6">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                          <span className="font-bold text-slate-800 text-lg">{item.projectClientName}</span>
                        </div>
