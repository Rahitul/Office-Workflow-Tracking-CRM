...
export async function GET(request: Request) {
  try {
    ...
    const targets = await BranchConsumableTarget.find(filter)
    ...
    console.error("Get branch consumable targets error:", error)
...
export async function POST(request: Request) {
  try {
    ...
    const target = await BranchConsumableTarget.findOneAndUpdate(
    ...
    console.error("Set branch consumable target error:", error)
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
}
