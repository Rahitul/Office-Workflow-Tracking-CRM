...
export default function CollectionTargetSettingPage() {
  const [loading, setLoading] = useState(false)
  const [existingTargets, setExistingTargets] = useState<CollectionTarget[]>([])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post("/api/collection-targets", {
        targetType: "company",
        targetId: "company",
      ...
      setSuccessMessage(`Collection target set successfully for ${MONTHS.find(m => parseInt(m.value) === parseInt(month))?.label} ${year}`)
      setAmountBDT("")
      fetchExistingTargets()
    } catch (error) {
      const err = error as { response?: { data?: { error?: string } } }
      setErrorMessage(err.response?.data?.error || "Failed to set collection target")
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleEdit = (target: CollectionTarget) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await axios.delete(`/api/collection-targets/${id}`, { withCredentials: true })
      fetchExistingTargets()
    } catch (error) {
  ...
  const handleSaveEdit = async (id: string) => {
    try {
      await axios.put(`/api/collection-targets/${id}`, { amountBDT: parseFloat(editAmount) }, { withCredentials: true })
      setEditingId(null)
      fetchExistingTargets()
    } catch (error) {
  ...
  return (
    <div className="space-y-6 pb-12">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Collection Target Setting</h1>
        <p className="text-slate-500">Set monthly collection target amount</p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div className="flex items-end">
                <Button
                  type="submit"
                  disabled={loading || !month || !year || !amountBDT}
      ...
      {existingTargets.length > 0 && (
        <Card className="border-slate-200 shadow-lg bg-white">
          <CardHeader className="border-b border-slate-100">
            <CardTitle className="flex items-center gap-2">
              <Target className="w-5 h-5 text-amber-600" />
              Existing Collection Targets ({existingTargets.length})
            </CardTitle>
          </CardHeader>
          <CardContent className="p-6">
