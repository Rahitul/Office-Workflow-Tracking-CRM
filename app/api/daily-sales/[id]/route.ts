...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const entry = await DailySalesEntry.findByIdAndUpdate(
      id,
      { 
    ...
    console.error("Failed to update daily sales entry:", error)
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const entry = await DailySalesEntry.findByIdAndDelete(id)
    ...
    console.error("Failed to delete daily sales entry:", error)
