...
export default function LogisticsJuniorsLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (mounted && !useAuthStore.getState().isLoading && !user) {
      router.push("/login")
    } else if (mounted && user && user.role !== "logistics_juniors" && user.role !== "admin") {
  ...
  const navItems = [
    { href: "/logistics_juniors/lead-transfer", label: "Lead Transfer", icon: ArrowRightLeft },
    { href: "/logistics_juniors/profile", label: "Profile", icon: User },
  ]
